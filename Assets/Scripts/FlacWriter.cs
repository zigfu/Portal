...
namespace Wav2Flac
{
    class FlacWriter : IDisposable
    {
        ...
        [DllImport(Dll)]
        static extern bool FLAC__stream_encoder_process_interleaved(IntPtr context, int[] buffer, int samples);
        ...
        public void Write(byte[] buffer, int offset, int uncompressedBytes)
        {
            ...
            //{
                //IntPtr input = new IntPtr(padded);

                Check(
                    FLAC__stream_encoder_process_interleaved(context, padded, samples),
